...
void App::OnLaunched(LaunchActivatedEventArgs const&)
{
    ...
    // Check if we have files to rename (either from pipe or command line)
    bool hasFilesToRename = (pipePos != std::wstring::npos) || !cmdLineFiles.empty();
    
    if (!hasFilesToRename)
    {
        // No files provided - launch Management Window
        Logger::debug(L"Starting PowerRename Management Window (no files provided)");
        m_managementWindow = make<ManagementWindow>();
        m_managementWindow.Activate();
        return;
    }
    
    // Files provided - launch Rename Window
